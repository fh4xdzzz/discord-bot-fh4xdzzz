...
class VerificationView(discord.ui.View):
    ...
    async def verify_button_callback(self, interaction: discord.Interaction):
        ...
        # Buscar rol de verificación (prioridad: servidor.config -> servidor -> global)
        server_verification_role_id = data['servers'][server_id].get('config', {}).get('verification_role')
...
def create_ranking_embed(guild_id):
    ...
    sorted_users = sorted(users.items(), key=lambda x: (x[1]['level'], x[1]['xp']), reverse=True)[:5]
...
async def create_stats_channel(interaction: discord.Interaction):
    try:
        # Crear canal de voz para mostrar miembros al principio
        ...
        stats_channel = await interaction.guild.create_voice_channel(
            ...
            overwrites={
                ...
                interaction.guild.owner: discord.PermissionOverwrite(
                    connect=False,  # Ni el dueño puede entrar
                    speak=False,
                    stream=False,
                    use_voice_activation=False
                )
        ...
        logger.info(f'Canal de estadísticas de voz creado al principio en servidor {interaction.guild.name}')
    except Exception as e:
